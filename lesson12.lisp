...
;;(load (merge-pathnames "lesson11.lisp"  *d*))
...
(set-alist-value :qux :someething *my-alist*) ;;error
...
;;; varying rotation for each shape by setting animator data
	...
	(add-animator *scene*
		      (make-instance 'animator
				     ...
				     :update-fn (lambda (anim)
						  (rotate-by (shape anim)
							     (anim-data anim :rotate)))
				     :data `((:rotate . ,(rand1 7)))))))
    :test #'is-leaf?))
...
(let ((start-pos (p! -.5 -.5))
  ...
  (set-animator *scene*
		(make-instance 'animator
			       ...
			       :update-fn (lambda (anim)
					    (translate-to *shape*
							  (p-smooth-lerp ;ease in/out -compare p-larp
...
;;any function curve
(defun make-func-shape (fn &key (num-points 64) (x-start 0.0) (x-end 1.0) (ky 1.0))
  (let ((shape (make-instance 'polygon-shape :is-closed-shape? nil)))
    (dotimes (i (+ num-points 1))
      (let ((x (+ x-start (* i (/ (- x-end x-start) num-points)))))
	(add-point shape (p! x (* ky (funcall fn x))))))
    shape))
...
(with-clear-and-redraw
  (add-shape *scene* (make-func-shape
                         (lambda (x) (* x x x)))))
(with-clear-and-redraw
  (add-shape *scene* (make-func-shape
                      (lambda (x) (- (* x x x) 2.0))
...
;;scale curve
(points (first (shapes *scene*))) 
(with-redraw
  (dolist (s (shapes *scene*))
    (scale-by  (transform s) 0.5)))
(with-redraw
  (dolist (s (shapes *scene*))
    (scale-by  (transform s) 2.0)))
||#
...
#||
(pprint (macroexpand-1 '(def-keyframe-animator translate)))
(pprint (macroexpand '(def-keyframe-animator translate)))
||#

(def-keyframe-animator translate) ;;make-translator-animator
...
(defparameter *shape*
  (with-clear-and-redraw
    (add-shape *scene* (make-square-shape 0.5))))

(progn
  (clear-animators *scene*)
  (add-animator *scene* (make-translate-animator
			 *shape* (p! -.5 -.5) (p! .5 .5) 0 60 :interpolation :smooth))
...
(progn
  (clear-animators *scene*)
  (add-animator *scene* (make-translate-animator
			 *shape* (p! -.5 -.5) (p! .5 .5) 0 60 :interpolation   :linear))
...
(cffi:translate-name-to-foreign 'clear-buffer :gl) ;"clear_buffer"
(uiop:split-string "asStreName" :separator #'upper-case-p) ; не работает
(require :str)
(str:split "asStreName" :separator #'upper-case-p) ;;не работает
(require :cl-ppcre)
(cl-ppcre:split "[A-Z]" "asStreName") ;;("as" "tre" "ame")
(cl-ppcre:regex-replace-all "(.)([A-Z])" "asStreName" "\\1-\\2") ;"as-Stre-Name"
(string-downcase
 (cl-ppcre:regex-replace-all "(.)([A-Z])" "asStreName" "\\1-\\2")) ;"as-stre-name"

(defmacro _glColor3f (x y z &optional alpha)
  (if alpha
      `(gl:color ,x ,y ,z ,alpha)
      `(gl:color ,x ,y ,z)))


(pprint (macroexpand-1 '(_glcolor3f 1.0 .5 .8)))
(pprint (macroexpand-1 '(_glcolor3f 1.0 .5 .8 1.0)))
