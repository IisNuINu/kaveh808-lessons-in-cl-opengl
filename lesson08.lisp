...
;;(load (merge-pathnames "lesson07.lisp"  *d*))
...
#||
;; функции позволяющие обновлять объекты при изменении определения класса
...
(with-redraw
   (randomize-colors-range *shape* (c! 0.0 0.0 0.0 0.25) (c! 0.0 0.0 1.0 0.25)))
(with-redraw
   (randomize-colors-range *shape* (c! 0.0 0.0 0.0) (c! 0.0 0.0 1.0)))
||#
...
(defun make-random-squares (n)
   (scatter-group (lambda () (make-square-shape (rand2 0.1 0.4)))
...
;;can't do with set-fill-color --set alpha only
(defun set-group-alpha (group alpha)
   (do-hierarhy group
      (lambda (s)
         (setf (aref (fill-color (appearance s)) 3) alpha))
      :test #'is-leaf?))

#||
(with-redraw
   (set-group-alpha *shape* 0.7))
...
;;unify setting colors, jittering color, add setting alpha
...
(defgeneric set-shape-color (self value-or-fn &rest args)
   ...
   (:method ((self t) value-or-fn &key (component :rgba)
                                    (part :fill) (operation :set))
      (declare (ignore value-or-fn component part operation)))

   (:method ((self appearance) value-or-fn &key (component :rgba)
                                    (part :fill) (operation :set))
   ...
   (:method ((self polygon-shape) value-or-fn &key (component :rgba)
                                                (part :fill) (operation :set))
      (set-shape-color (appearance self) value-or-fn :component component
                       :part part :operation operation))

   (:method ((self group) value-or-fn &key (component :rgba)
                                                (part :fill) (operation :set))
      (dolist (child (children self))
         (set-shape-color child value-or-fn
                          :component component
                          :part part :operation operation)))
   )

;;;; redefine in terms of set-shape-color
;;; пепеопределим в терминах set-shape-color
(defun randomize-colors (shape)
   (set-shape-color shape #'c-rand))

(defun randomize-colors-range (shape c1 c2)
   (set-shape-color shape
                    (lambda () (c-rand2 c1 c2)) :component :rgb))

(defun jitter-shape-color (shape c-delta)
  (set-shape-color shape
		   (lambda () (c-rand1 c-delta)) :component :rgb :operation :add))
...
(with-redraw
  (randomize-group-points *shape* 0.05))
  
(with-redraw
  (do-hierarhy *shape*
    (lambda (shape)
      (randomize-size shape 0.8 1.5))
    :test #'is-leaf?))
...
(with-redraw
  (randomize-colors-range *shape* (c! 0 0 0) (c! 1 0 0)))
(with-redraw
  (set-shape-color *shape* 0.5 :component :alpha))
(with-redraw
...
(defun make-cloud-group (num shape-fn bounds-lo bounds-hi &key (max-alpha 0.25) (scale? nil))
  (let ((group (scatter-group shape-fn
    ...
    (dolist (shape (children group))
      (let* ((factor (- 1.0 (/ (p-mag (translate (transform shape)))
			       max-dist)))
